...
func (c Color) Scale(k float32) Color {
...
func (c Color) toDrawingColor() Color {
	legalize := func(n float32) float64 {
		if n > 1.0 {
			return 1.0
		} else {
			return float64(n)
	...
	return Color{
		r: float32(math.Floor(legalize(c.r) * 255.0)),
		g: float32(math.Floor(legalize(c.g) * 255.0)),
		b: float32(math.Floor(legalize(c.b) * 255.0)),
	}
}
