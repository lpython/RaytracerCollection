...
export default class RayTracer {
    ...
    intersections(ray, scene) {
        ...
        // {
        //     let hit = (closestInter !== undefined)
        //     Log("Final | Hit:", hit)
        // }
        return closestInter;
    }
    ...
    getNaturalColor(thing, pos, norm, rd, scene) {
        ...
        // let c = scene.lights.reduce(addLight, Color.defaultColor);
        let c = scene.lights.reduce((acc, light) => {
            return addLight(acc, light)
        }, Color.defaultColor);
        return c;
    }
    ...
    renderToImage(scene, image) {
        ...
        for (var y = 0; y < height; y++) {
            for (var x = 0; x < width; x++) {
                ...
                let c = this.traceRay({ start: scene.camera.pos, dir: getPoint(x, y, scene.camera) }, scene, 0);
                c = Color.toDrawingColor(c);

                // let ray = { start: scene.camera.pos, dir: getPoint(x, y, scene.camera) };
                // Log("Scene.Camera: ", scene.camera);
                // Log(`X:${x} Y:${y}`);
                // Log("Ray:", ray.start, ray.dir);
                // Log("Float color : ", c);
                // Log("UINT color : ", c2);
